...
function answered(instance, attached, request){
  ...
  if(instance.href){window.location=instance.href;}else if(instance.action && instance.push == undefined){ 
    ...
    console.log("relying on addEvent to call properly");
    //alert("waittt");
  }
...
function formAttach()
{
        ...
        for (counter=0; counter<forms.length; counter++)
        {
          ...
          if((forms[counter].className != "acskip") && (forms[counter].action != window.location) && (forms[counter].action.indexOf("clickfunnels.com") == -1) && (forms[counter].action.indexOf("/members") == -1)){
            ...
            console.log("eval "+forms[counter].id);
            ...
            addEvent(forms[counter], 'submit', function(){console.log("sending data - counter: "+counter+" - forms: "+document.getElementsByTagName('form') + " - id: "+document.getElementsByTagName('form')[counter].id);SendData(forms[counter].id, forms[counter].onsubmit);console.log("sent data");return true;});
            
            // TODO FIX to only use addEvent - make work w/ scripts that should be stopping form submission like validates jquery

            console.log(forms[counter].elements);
...
function addEvent(obj, evType, fn) {
    if (obj.addEventListener) {
        ...
        console.log("Handler could not be attached cleanly");
    }
}
...
function cf_load(){
  ...
  if(readCookie(page_key) == 'true' || readCookie(fid.toString()+'_viewed_'+(fspos).toString()) == fvrs.toString()){
    ...
    if(typeof(fid) != 'undefined'){
      if(readCookie(fid.toString()+'_viewed_'+(fspos-1).toString()) == fvrs.toString() && readCookie(fid.toString()+'_viewed_1').toString() == fvrs.toString() ){
